import *  as constant from '../Action/contsant'
...
export default function productreducer(state = initialstate, action) {
    switch (action.type) {
        case constant.GET_PRODUCT_SUCCESS:{
            ...
            // draft.product = {}
            // return draft;
            return {...state,product:{},allproduct:action.payload}; 
        }
        ...
        // add product
            ...
            case constant.ADD_PRODUCT_SUCCESS:{
                // const draft = state;
                // draft.allproduct = [...draft.allproduct,action.payload];
                // draft.product = {}
                // return draft;
                let allproduct = state.allproduct;
                allproduct.push(action.payload);
                return {...state, product:{},allproduct:allproduct};
            }

            // update product
            case constant.UPDATE_PRODUCT_SUCCESS:{
            //     const draft = state;
            //   const index = draft.allproduct.findIndex((d)=>d.id===action.payload.id) || -1
            //   draft.allproduct[index]=action.payload
            //   return draft
            const index = state.allproduct.findIndex((d)=>d.id===action.payload.id) || -1
            ...
            case constant.DELETE_PRODUCT_SUCCESS:{
                // const draft = state;
            //   draft.allproduct = draft.allproduct.filter((d)=>d.id !==action.payload)
            //   return draft
            let allproduct =state.allproduct.filter((d)=>d.id !== action.payload)
            ...
            //     const draft = state;
            //  const index = draft.allproduct.findIndex((d)=>d.id===action.payload) || -1
            //  draft.product=draft.allproduct[index]
            //   return draft;

            const index = state.allproduct.findIndex(d=> d.id === action.payload);
            const product = state.allproduct[index];
